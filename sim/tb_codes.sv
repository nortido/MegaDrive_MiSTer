...
module tb_codes;
	...
	// the table changes under a still bus: the output may lag a clock, but must be
	// the plain bus word or the original's answer
	reg watch = 0;
	integer lag = 0;
	...
	initial begin
		...
		// a word code at an even address and a byte code at the odd one above write
		// the same half, and the later code has to win
		...
		// reset while a code hits, then the code again, then with a failing compare
